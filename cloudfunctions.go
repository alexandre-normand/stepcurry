...
func init() {
	...
	slackToken, slackSigningSecret, fitbitClientID, fitbitClientSecret, err := loadSecrets(projectID)
	if err != nil {
	...
	roger, err := New(inferBaseURL(projectID, region), fitbitClientID, fitbitClientSecret, OptionSlackVerifier(slackSigningSecret), OptionStorer(storer), OptionUserInfoFinder(slackClient), OptionMessenger(slackClient), OptionChannelInfoFinder(slackClient), OptionTaskScheduler(taskScheduler))
	if err != nil {
...
func loadSecrets(projectID string) (slackToken, slackSigningSecret, fitbitClientID, fitbitClientSecret string, err error) {
...
func StartFitbitOauthFlow(w http.ResponseWriter, r *http.Request) {
...
func HandleFitbitAuthorized(w http.ResponseWriter, r *http.Request) {
	rc.HandleFitbitAuthorized(w, r)
}

func ChallengeStandings(w http.ResponseWriter, r *http.Request) {
